...
// move. onground is set for toss objects when they come to a complete rest. it
...
{ // Slide off of the impacting object
	...
	if(!normal[2]) blocked |= 2; // step/wall
...
s32 SV_FlyMove(edict_t *ent, f32 time, trace_t *steptrace)
{
	...
	s32 numplanes = 0;
	f32 time_left = time;
	for(s32 bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		if(!ent->v.velocity[0] && !ent->v.velocity[1]
				&& !ent->v.velocity[2]) break;
		for(s32 i = 0; i < 3; i++)
		...
		SV_Impact(ent, trace.ent); // run the impact function
		if(ent->free) break; // removed by the impact function
		time_left -= time_left * trace.fraction;
		if(numplanes >= MAX_CLIP_PLANES) { // cliped to another plane
			VectorCopy(vec3_origin, ent->v.velocity);
			return 3;
		...
		if(i != numplanes) { // go along this plane
			...
			f32 d = DotProduct(dir, ent->v.velocity);
...
void SV_PushMove(edict_t *pusher, f32 movetime)
{
	...
	for(e=1 ; e<qcvm->num_edicts ; e++, check = NEXT_EDICT(check))
	{
		...
		if( ! ( ((s32)check->v.flags & FL_ONGROUND)
		...
		if(check->v.movetype != MOVETYPE_WALK)
			check->v.flags = (s32)check->v.flags & ~FL_ONGROUND;
...
void SV_CheckStuck(edict_t *ent) // This is a big hack to try and fix the rare
{ // case of getting stuck in the world clipping hull.
	s32 i, j;
	s32 z;
	vec3_t org;
...
/*
   ...
   ======================
   */
s32 SV_TryUnstick(edict_t *ent, vec3_t oldvel)
{
	vec3_t oldorg, dir;
	...
	for(s32 i = 0; i < 8; i++) {
		...
		ent->v. velocity[2] = 0;
		trace_t steptrace;
		s32 clip = SV_FlyMove(ent, 0.1, &steptrace);
...
{ // Only used by players
	...
	s32 oldonground = (s32)ent->v.flags & FL_ONGROUND;
	ent->v.flags = (s32)ent->v.flags & ~FL_ONGROUND;
	vec3_t oldorg, oldvel;
	VectorCopy(ent->v.origin, oldorg);
	VectorCopy(ent->v.velocity, oldvel);
	trace_t steptrace, downtrace;
	s32 clip = SV_FlyMove(ent, host_frametime, &steptrace);
	if(!(clip & 2)) return; // move didn't block on a step
	if(!oldonground&&ent->v.waterlevel==0) return; //don't stair up mid-jump
	if(ent->v.movetype != MOVETYPE_WALK) return; // gibbed by a trigger
	if(sv_nostep.value) return;
	if((s32)sv_player->v.flags & FL_WATERJUMP) return;
	...
	SV_PushEntity(ent, upmove); // move up
	ent->v.velocity[0] = oldvel[0]; // move forward
	ent->v. velocity[1] = oldvel[1];
	ent->v. velocity[2] = 0;
...
void SV_Physics_Client(edict_t *ent, s32 num) // Player character actions
{
	...
	switch((s32)ent->v.movetype) { // decide which move function to call
	case MOVETYPE_NONE:
		if(!SV_RunThink(ent)) return;
		break;
	case MOVETYPE_WALK:
		if(!SV_RunThink(ent)) return;
		if(!SV_CheckWater(ent) && ! ((s32)ent->v.flags & FL_WATERJUMP))
			SV_AddGravity(ent);
		SV_CheckStuck(ent);
		SV_WalkMove(ent);
		break;
	...
	case MOVETYPE_NOCLIP:
	   ...
	   VectorMA(ent->v.origin,host_frametime,ent->v.velocity,ent->v.origin);
		break;
	default:
	   Sys_Error("SV_Physics_client: bad movetype %i",(s32)ent->v.movetype);
	}
...
{
	s32 cont = SV_PointContents(ent->v.origin);
...
{ // Toss, bounce, and fly movement. When onground, do nothing.
	vec3_t move;
	f32 backoff;
	if(!SV_RunThink(ent)) return; // regular thinking
	// if onground, return without moving
	if(((s32)ent->v.flags & FL_ONGROUND)) return;
	...
	trace_t tr = SV_PushEntity(ent, move);
	if(tr.fraction == 1) return;
	if(ent->free) return;
	if(ent->v.movetype == MOVETYPE_BOUNCE) backoff = 1.5;
	else backoff = 1;
	ClipVelocity(ent->v.velocity, tr.plane.normal, ent->v.velocity,backoff);
	// stop if on ground
...
{
	bool hitsound; // freefall if not onground
	if(!((s32)ent->v.flags & (FL_ONGROUND | FL_FLY | FL_SWIM))) {
		if(ent->v.velocity[2] < sv_gravity.value*-0.1) hitsound = 1;
		else hitsound = 0;
		SV_AddGravity(ent);
		SV_CheckVelocity(ent);
		SV_FlyMove(ent, host_frametime, NULL);
